...
Supported types: regions, subregions, cities, industries, country, timezones, postal_codes
...
Example usage:
    ...
    ./manage.py generate_json --postal_codes --source geonames
"""
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        group.add_argument(
            ...
            const='postal_codes',
            dest='type',
            help="Process postal codes data"
        )
        parser.add_argument(
            "--source",
            type=str,
    ...
    def validate_entry(self, entry: dict, input_type: str, line_num: int) -> tuple[dict | None, tuple[str, str] | None]:
        ...
        elif input_type == 'postal_codes':
            if not entry.get('postal_code') or not entry.get('country_code'):
        ...
        try:
            ...
            for idx, column in enumerate(columns):
                ...
                if input_type in ['regions', 'subregions'] and column == 'code_field':
                    ...
                    if column in ['place_name', 'admin_name1', 'admin_name2', 'admin_name3']:
                        entry[column] = normalize_text(value) if value else None
                    elif column in ['latitude', 'longitude']:
                        entry[column] = float(value) if value and value.replace('.', '', 1).replace('-', '', 1).isdigit() else None
                    else:
                        entry[column] = value if value else None
                elif column in self.CONFIG[input_type]['json_columns']:
                    if column in ['name', 'asciiname', 'place_name', 'admin_name1', 'admin_name2', 'admin_name3', 'alternatenames']:
                        entry[column] = normalize_text(value) if value else None
                    elif column in ['latitude', 'longitude']:
                        entry[column] = float(value) if value and value.replace('.', '', 1).replace('-', '', 1).isdigit() else None
                    else:
    ...
    def process_timezones_txt(self, input_file: str, output_file: str, start_time: float) -> dict:
        ...
        try:
            ...
            if timezones:
                try:
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump(timezones, f, cls=RawStringEncoder, indent=4, ensure_ascii=False)
                    self.stdout.write(self.style.SUCCESS(f"Generated {output_file} with {stats['processed']} entries ({time.time() - start_time:.2f}s)"))
                    logger.info(f"Generated {output_file} with {stats['processed']} entries")
                except Exception as e:
                    self.stderr.write(self.style.ERROR(f"Error writing to {output_file}: {str(e)}"))
    ...
    def handle(self, *args, **options) -> None:
        ...
        env_mapping = {
            ...
            'postal_codes': ('POSTAL_CODES_TXT', 'POSTAL_CODES_JSON')
        }
